 ...
 * linear scan is plenty quick for the size we ship.  audit 07 expanded
 * from ~50 to ~120 entries covering modern IoT and consumer devices.
 *
 * Source: hand-picked from the public IEEE OUI registry plus
 * Wireshark's manuf file (most-frequently-seen first within each vendor).
 * If a vendor has multiple OUIs we pick 1-2 of the most common rather than
 * exhaustively listing every prefix. */
