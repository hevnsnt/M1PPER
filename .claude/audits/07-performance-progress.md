...
| 8 | TV-B-Gone code DB from SD (app_tvbgone.c:73-96) | DONE | Commit b9eaeba (Phase 5 absorbed it on the third attempt).  At app entry the file `/IR/tvbgone.txt` is opened; format `BRAND,PROTOCOL,ADDRESS,COMMAND,REPEATS` (one entry per line, '#' and blank lines skipped).  Address/command accept `0x`-prefixed hex or decimal.  On any failure (no SD, missing file, parse error) the hardcoded 16-brand list remains active. |
| 9 | Net Recon larger OUI + ports (app_net_recon.c) | DONE | Commits b910414 (ports) + a7a533f (OUI).  Top-30 port list with modern IoT defaults (5000, 8000, 8888, 631, 9100, 1883, 8883, 5353, 1900, 139), reordered for likely-positive-first.  OUI table expanded from ~52 to ~95 entries covering Espressif (3 prefixes), Roku (3), Sonos (4), Apple (~25), TP-Link (4), Belkin, Eero, Ring, Tuya, Hon Hai/Foxconn, Synology, Sercomm and other consumer IoT vendors (~600 B flash cost). |
